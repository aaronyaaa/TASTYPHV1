function initSellerMap(
  ...
  if (
    ...
    const distance = parseFloat(
      haversineDistance(sellerLat, sellerLng, userLat, userLng)
    );
    const routeColor =
      distance > 20 ? "red" : distance > 5 ? "orange" : "green";
    const estimatedTime = ((distance / 40) * 60).toFixed(0); // 40 km/h average speed

    const map = L.map("map").setView(
      [(sellerLat + userLat) / 2, (sellerLng + userLng) / 2],
      13
    );
    ...
    L.marker([sellerLat, sellerLng], { icon: sellerIcon })
      .addTo(map)
      .bindPopup("🏪 <strong>Seller</strong><br>" + sellerAddress);
    ...
    polyline
      .bindPopup(
        `<strong>📏 Distance:</strong> ${distance} km<br>⏱ ETA: ${estimatedTime} mins`
      )
      .openPopup();
    ...
    const motorMarker = L.rotatedMarker([sellerLat, sellerLng], {
      icon: motorIcon,
      rotationAngle: getBearing(sellerLat, sellerLng, userLat, userLng),
    ...
    const animate = () => {
      ...
      progress += 0.003;
      const lat = sellerLat + (userLat - sellerLat) * progress;
      const lng = sellerLng + (userLng - sellerLng) * progress;
